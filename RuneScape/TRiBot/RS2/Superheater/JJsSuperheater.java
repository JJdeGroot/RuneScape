...
@ScriptManifest(authors = { "J J" }, category = "Magic", name = "JJ's Superheater V0.2")
public class JJsSuperheater extends Script implements Painting{
	JJsSuperheaterDTMs dtms = new JJsSuperheaterDTMs();
	JJsSuperheaterGUI gui = new JJsSuperheaterGUI();
	JJsSuperheaterPrices ge = new JJsSuperheaterPrices();
	...
	// Waits until a specific tab is visible
	...
	private boolean openBankNPC(){
		long[] NPCs = {964016324L, 186323308L, 2791056588L, 372895788L, 
				   	   3790880451L, 4001282980L, 1919077414L, 558125369L, 
					   666845606L, 4050299616L, 4222475077L, 901358828L,
					   3365752498L, 2129181768L, 666845606L, 4222475077L, 
					   1676840816L, 3337621782L, 1461229868L, 2600562223L, 
					   2553342430L, 3273447472L, 1775775211L, 2086701374L, 
					   1500860893L, 2086701374L, 3666623099L, 3939305803L,
					   310199159L};
		...
		if (ID != -1){
			...
			if (P != null){
				Point BP = new Point(P.x + randomRange(-6, 6), P.y - randomRange(5, 10));
				Mouse.move(BP);
	...
	private boolean openBankChest(){
		...
		if (ID != -1){
			Point P = getNearestPoint(ID);
			if (P != null){
				Point CP = new Point(P.x + randomRange(-8, 8), P.y + randomRange(-6, 6));
				Mouse.move(CP);
				if (Timing.waitUptext("Bank chest", 500)){
					Mouse.click(1);
					return true;
				}else{
					if (Timing.waitUptext("Superheat", 500)){
						Mouse.click(3);
						if (Timing.waitChooseOption("Bank", 500)){
							return true;
						}
	...
	private boolean openNormalBank(){
		...
		long[] NPCs = {964016324L, 186323308L, 2791056588L, 372895788L, 
					   ...
					   666845606L, 4050299616L, 4222475077L, 901358828L,
					   3365752498L, 2129181768L, 666845606L, 4222475077L, 
					   1676840816L, 4216896244L, 3337621782L, 1461229868L,
					   2600562223L, 2553342430L, 3273447472L, 1775775211L, 
					   2086701374L, 1500860893L, 2086701374L, 3666623099L,
					   3939305803L, 310199159L};
			...
			if (PP != null){
				...
				if (ID != -1){
					...
					if (P != null){
						...
						if (xDist > yDist){
							BP = new Point(P.x + randomRange(-12, 12), PP.y + randomRange(-8, 8));
						}else{
							BP = new Point(PP.x + randomRange(-12, 12), P.y + randomRange(-8, 8));
						}
	...
	// Method to handle the banking
	private void bankingMainloop(){
		if (openBank()){
			if (waitBankscreen(5000)){
				if (deposit()){
					withdrawOres(bar);
	...
	// Method to handle the superheating
	private void superheatingMainloop(){
		long maxTime = System.currentTimeMillis() + 60000;
		
		while (ores > 1 && System.currentTimeMillis() < maxTime){
			clickSuperheat();
			ores = canSuperheat();
			doSuperheat();
		}
