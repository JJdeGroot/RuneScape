...
import org.tribot.api.Timing;
    ...
    public class JJsGnome3 extends Script implements Painting {
        ...
        private Point getNearest(long[] ids){
        	ScreenModel[] allIDs = ScreenModels.getAll();
        	...
        	for (int i=0; i<allIDs.length; i++)
        		for (int j=0; j<ids.length; j++)
        			if (allIDs[i].id == ids[j])
        				if (allIDs[i].base_point.x > 50 && allIDs[i].base_point.x < 480)
        					if (allIDs[i].base_point.y > 100 && allIDs[i].base_point.y < 340){
        						idList.add(ids[j]);
                				distList.add((int) Math.sqrt(Math.pow(Math.abs(middleMS.x - allIDs[i].base_point.x), 2) + Math.pow(Math.abs(middleMS.y - allIDs[i].base_point.y), 2)));
                				coordList.add(new Point(allIDs[i].base_point.getLocation()));
        					}
	    ...
	    public void run() {
            ...
            //long[] id = {1107874667L};
        	//Mouse.move(getNearest(id));
        	//Mouse.move(getNearest(obstacles));
