...
public class Walking {
	...
	private final static Location[] locations = {Location.AIR_NORMAL, Location.AIR_GOP, 
					...
					Location.BODY_GOP, Location.LOBBY};
	...
	private static int countBlackPixels(){
		...
		for(int x = 0; x < data.length; x++){
			for(int y = 0; y < data[x].length; y++){
				...
				if(Screen.coloursMatch(c1, c2, tol)){
					count++;
				}
			}
		}
		
		//System.out.println("Black pixels: " + count);
		return count;
	}
	...
	public static Location getLocation(){ 
		int count = countBlackPixels(),
			smallest = Math.abs(blackCounts[0]-count),
			position = 0;

		for(int i = 1; i < blackCounts.length; i++){
			int difference = Math.abs(blackCounts[i] - count);
		...
		if(!myLoc.equals(Location.LOBBY)){
			for(int i = 0; i < locations.length; i++){
				if(myLoc.equals(locations[i])){
					...
					if(difference > 10){
						int amount = (int) Math.round(difference/10);
