...
public class Portal {
	...
	private final static String portalString = "Enter Portal",
								waitForPortalString = "portal",
								waitForOrbsString = "orbs",
								waitForPlayerString = "players",
								portalIsOpenString = "portal";
	...
	public static boolean waitForPortalText(){
		return waitForText(new Rectangle(175, 329, 200, 15), waitForPortalString);
	}

	// Reads the waiting for orbs text
	public static boolean waitForOrbsText(){
		return waitForText(new Rectangle(175, 329, 200, 15), waitForOrbsString);
	}
	
	// Reads the waiting for players text
	public static boolean waitForPlayersText(){
		return waitForText(new Rectangle(175, 329, 200, 15), waitForPlayerString);
	}

	// Reads the is the portal open text
	public static boolean isOpen(){
		return waitForText(new Rectangle(208, 259, 140, 16), portalIsOpenString);
	}
	...
	public static boolean enterPortal(){
		int count = 0;
		while(count < 3){
			if(DynamicClicking.clickScreenModel(General.getScreenModel(portalID), portalString)){
