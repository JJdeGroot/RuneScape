...
@ScriptManifest(authors = { "J J" }, category = "RuneCrafting", name = "JJ's Air RCer 07 v2") // Credits to firebird891 and Will
public class JJsAirRcerv2 extends Script implements Painting {
	...
	public void onPaint(Graphics g) {
		...
		g.drawString("JJ's Air RCer v2", 10, 20);
	...
	private boolean atBank(){
		return Objects.find(8, BANK_BOOTH_ID).length > 0;
	}
	...
	private boolean atRuins(){
		return Objects.find(8, AIR_RUINS_ID).length > 0;
	}
	
	private boolean atAltar(){
		return Objects.find(8, AIR_ALTAR_ID).length > 0;
	}
	
	private boolean atPortal(){
		return Objects.find(8, AIR_PORTAL_ID).length > 0;
	}
	...
	private boolean walkToTile(RSTile tile){
		tile = new RSTile(tile.getX() + General.random(-3, 3), tile.getY() + General.random(-3, 3), tile.getPlane());
	...
	public void run() {
		...
		while(true) {
			...
			if (Camera.getCameraRotation() != 137)
